...
int main(void)
{
  double t_sum,t_min,t_max,t[6];
  double m_sum,m_min,m_max,m[6];
  double *a,*b,*c,dt;
  function_t *f;
  int i,j,n;

  perf_counters_init();
  perf_counters[0].config = PERF_COUNT_HW_CACHE_REFERENCES;
  perf_counters[1].config = PERF_COUNT_HW_CACHE_MISSES;
  ...
  for(n = 16 * 0 + N - 4;n <= N;n++)
  {
    ...
    for(i = 0;i < 6;i++)
    {
      ...
      for(j = 0;j < 6;j++)
      {
        ...
        if(j == 0 || dt > t_max)
          t_max = dt;
        // cache misses percentage
        if(perf_counters[0].value == 0ul)
          perf_counters[0].value = 1ul;
        dt = 100.0 * (double)perf_counters[1].value / (double)perf_counters[0].value;
        m_sum += dt;
        if(j == 0 || dt < m_min)
          m_min = dt;
        if(j == 0 || dt > m_max)
      ...
      m[i] = (m_sum - m_min - m_max) / 4.0;
    }
    // report
    printf("%4d %7.3f %6.1f %7.3f %6.1f %7.3f %6.1f %7.3f %6.1f %7.3f %6.1f %7.3f %6.1f\n",n,t[0],m[0],t[1],m[1],t[2],m[2],t[3],m[3],t[4],m[4],t[5],m[5]);
  }
  printf("#--- ------- ------ ------- ------ ------- ------ ------- ------ ------- ------ ------- ------\n");
  // clean up and exit
  free(a);
